...
server <- function(input, output, session) {
  ...
  p_list <- reactiveVal(character(0))
  c_list <- reactiveVal(character(0))
  o_list <- reactiveVal(character(0))
  f_list <- reactiveVal(character(0))
  g_list <- reactiveVal(character(0))
  s_list <- reactiveVal(character(0))
  ...
  observeEvent(input$kingdom_update,{
    ...
    updateCheckboxGroupInput(session, "phylumGroup", choices = p_list.new)
    updateCheckboxGroupInput(session, "classGroup", choices = c_list.new)
    updateCheckboxGroupInput(session, "orderGroup", choices = o_list.new)
    updateCheckboxGroupInput(session, "familyGroup", choices = f_list.new)
    updateCheckboxGroupInput(session, "genusGroup", choices = g_list.new)
    updateCheckboxGroupInput(session, "speciesGroup", choices = s_list.new)
  ...
  observeEvent(input$phylum_selectall,{
    print(input$phylum_selectall)
    if(input$phylum_selectall%%2 == 0){
      updateCheckboxGroupInput(session,"phylumGroup",choices=p_list())
    }
    else{
      updateCheckboxGroupInput(session,"phylumGroup",choices=p_list(), selected=p_list())
    }
  })
